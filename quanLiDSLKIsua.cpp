...
int maxDiem(LINKEDLIST myList);
int minDiem(LINKEDLIST myList);
void xuLy(LINKEDLIST& myList);
...
void xuLy(LINKEDLIST& myList)
{
	for(NODE* p=myList.Head;p!=NULL;p=p->next)
	    if(p->next!=NULL&&p->next->diemCTDL==maxDiem(myList))
	    {
	       addFirst(myList,p->next);
	       removeNode(myList,p);
	    }
	for(NODE* p=myList.Head;p!=NULL;p=p->next)
	    if(p->next!=NULL&&p->next->diemCTDL==minDiem(myList))
	    {
	    	addLast(myList,p->next);
	    	removeNode(myList,p);
		}
